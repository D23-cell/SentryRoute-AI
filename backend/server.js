...
app.get("/api/logs", (req, res) => res.json(db.get("logs").value()));
...
app.post("/api/verify-otp", (req, res) => {
  ...
  if (otp === currentOTP) {
    const token = jwt.sign({ user: username, role }, SECRET_KEY, {
      expiresIn: "1h",
    });
    currentOTP = null;
    res.json({ token, role });
...
app.post("/api/execute-reroute", (req, res) => {
  ...
  res.json({
    message: "✅ Action Verified & Logged to Blockchain",
    block: auditEntry,
  });
});
...
app.get("/api/verify-chain", (req, res) => {
  ...
  if (!logs || logs.length === 0)
    return res.json({ status: "SUCCESS", message: "🔒 Blockchain is empty." });
  ...
  if (isChainValid)
    ...
    res
      .status(418)
      .json({ status: "CRITICAL", message: "🚨 Tampering Detected!" });
});
...
io.on("connection", (socket) => {
  ...
  const interval = setInterval(() => {
    socket.emit(
      "live-news-update",
      "📡 ALERT: New satellite imagery received.",
    );
  }, 15000);
...
server.listen(PORT, () =>
  console.log(`🚀 Strategic Server running on Port ${PORT}`),
);
